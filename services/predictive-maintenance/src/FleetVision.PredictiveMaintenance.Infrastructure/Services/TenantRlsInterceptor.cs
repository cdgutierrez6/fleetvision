...
public sealed class TenantRlsInterceptor : DbConnectionInterceptor
{
    ...
    public override async Task ConnectionOpenedAsync(
        ...
        => await SetTenantConfigAsync(connection, ct);

    public override void ConnectionOpened(DbConnection connection, ConnectionEndEventData eventData)
        => SetTenantConfigAsync(connection, CancellationToken.None).GetAwaiter().GetResult();

    private async Task SetTenantConfigAsync(DbConnection connection, CancellationToken ct)
    {
