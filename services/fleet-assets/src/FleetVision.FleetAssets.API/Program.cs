using FleetVision.FleetAssets.API;
using FleetVision.FleetAssets.API.Middleware;
using FleetVision.FleetAssets.Application.Vehicles.Commands;
using FleetVision.FleetAssets.Infrastructure;
...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("service", "fleet-assets")
...
builder.Services.AddOpenTelemetry()
    .ConfigureResource(r => r.AddService("fleetvision-fleet-assets", serviceVersion: "1.0.0"))
    .WithTracing(tracing =>
    {
        tracing
...
builder.Services.AddHealthChecks()
    .AddNpgSql(
        builder.Configuration.GetConnectionString("FleetAssetsDb")!,
        name: "postgres",
        tags: ["db", "ready"]);
...
if (app.Environment.IsDevelopment())
{
    ...
    var db = scope.ServiceProvider.GetRequiredService<FleetAssetsDbContext>();
    await db.Database.MigrateAsync();
    Log.Information("Database migrations applied");
}

Log.Information("FleetVision Fleet & Assets Service started on {Environment}", app.Environment.EnvironmentName);
app.Run();
