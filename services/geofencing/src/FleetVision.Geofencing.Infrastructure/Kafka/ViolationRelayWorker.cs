...
/// <summary>
/// Polls geofencing_outbox_events and publishes to Kafka topic "geofencing.violations".
/// At-least-once: if the process dies after outbox INSERT but before Kafka ACK,
/// the event is retried on the next cycle. Consumer must be idempotent.
/// Uses FOR UPDATE SKIP LOCKED to allow horizontal scaling without double-publish.
///
/// Events that exceed MaxRetries are permanently skipped. A cleanup job (every CleanupEvery
/// cycles) removes them after ExpiredRetentionDays to prevent unbounded table growth.
/// </summary>
public sealed class ViolationRelayWorker : BackgroundService
{
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<ViolationRelayWorker> _logger;
    private readonly IProducer<string, byte[]> _producer;
    private readonly TimeSpan _pollInterval = TimeSpan.FromMilliseconds(500);
    private const int BatchSize           = 100;
    private const int MaxRetries          = 10;
    // Run cleanup every 120 cycles ≈ every 60 seconds at 500 ms poll interval
    private const int CleanupEvery        = 120;
    private const int ExpiredRetentionDays = 7;
    private int _cycleCount               = 0;

    public ViolationRelayWorker(
        IServiceScopeFactory scopeFactory,
        ILogger<ViolationRelayWorker> logger,
    ...
    {
        _logger.LogInformation("ViolationRelayWorker started.");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "ViolationRelayWorker batch error. Retrying.");
            }
        ...
        _logger.LogInformation("ViolationRelayWorker stopped.");
    }
    ...
    private async Task ProcessBatchAsync(CancellationToken ct)
    {
        ...
        {
            try   { return (i, error: (string?)null, delivery: (DeliveryResult<string, byte[]>?)await t); }
            catch (Exception ex) { return (i, error: ex.Message, delivery: (DeliveryResult<string, byte[]>?)null); }
        }));
        ...
        if (successIds.Count > 0)
        {
            const string markSql = """
                UPDATE geofencing_outbox_events
        ...
        if (successIds.Count > 0)
            _logger.LogDebug("Relayed {Count} violation events to Kafka.", successIds.Count);
    }

    // Removes outbox events that exceeded MaxRetries and are older than ExpiredRetentionDays.
    // Runs periodically to prevent unbounded table growth when Kafka is persistently unavailable.
    private async Task PurgeExpiredAsync(CancellationToken ct)
    {
        ...
        const string purgeSql = """
            DELETE FROM geofencing_outbox_events
        ...
        if (deleted > 0)
            _logger.LogWarning(
                "Purged {Count} expired outbox events (retry_count >= {Max}).", deleted, MaxRetries);
    }
